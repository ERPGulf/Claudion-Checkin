...
/**
 * Create expense claim (multipart)
 * claimData: { expense_date, expense_type, amount, description, file_url: [{ uri, name, mimeType? }] }
 */
export const createExpenseClaim = async (claimData) => {
  try {
    const [rawBaseUrl, token, employee] = await Promise.all([
      AsyncStorage.getItem("baseUrl"),
      AsyncStorage.getItem("access_token"),
      AsyncStorage.getItem("employee_code"),
    ]);

    if (!rawBaseUrl || !token || !employee) {
    ...
    console.error("Error creating expense claim:", error.response?.data || error.message);
    throw error;
  }
};

/**
 * Upload file for expense claim
 * file: { uri, name, type }
 * docname: expense claim ID
 */
