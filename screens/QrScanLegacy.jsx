...
/**
 * TEMPORARY (New Home Experience experiment) — the classic Scan QR Code screen,
 * preserved exactly as it shipped.
 *
 * Moved here verbatim from QrScan.jsx when the modern scanner landed; QrScan.jsx
 * is now the container that picks between the two. Nothing in this file changed —
 * same permission effect, same `handleQRCodeData` parser character for character,
 * same `Camera.scanFromURLAsync` gallery path, same `alert()` failures, same
 * navigate("login"), same header. Light-only by design, which is why it reads
 * COLORS directly rather than useAppTheme().
 *
 * The modern screen runs the identical logic through hooks/useQrScanner.js — a
 * faithful lift of what this file still runs inline, exactly as MyQrCodeLegacy
 * and hooks/useQrCode.js relate. Keep the two in step if the parser ever changes.
 *
 * Delete this file with the rest of the experiment.
 */
function QrScanLegacy() {
